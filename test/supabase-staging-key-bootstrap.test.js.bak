...
assert.match(bootstrap, /Session Pooler connection on port 5432/);
assert.match(bootstrap, /databaseUser !== `postgres\.\$\{projectRef\}`/);
assert.match(bootstrap, /database connection preflight/);
assert.match(bootstrap, /select current_database\(\), current_user/);
assert.match(bootstrap, /result\.error/);
assert.match(bootstrap, /REDACTED_DATABASE_PASSWORD/);
assert.match(bootstrap, /STAGING_DATABASE_HOST/);
assert.match(bootstrap, /Staging \$\{phase\} failed with psql exit status/);
