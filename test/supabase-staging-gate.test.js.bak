...
const stagingRef = workflow.match(/SUPABASE_STAGING_PROJECT_REF:\s*([a-z0-9]+)/)?.[1];
assert.equal(stagingRef, 'kdpjlbqvdlnstfhrmchm');
assert.equal(stagingRef.length, 20, 'Supabase project refs must contain 20 characters.');
